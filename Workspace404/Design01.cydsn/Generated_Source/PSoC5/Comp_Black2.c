...
uint8 Comp_Black2_initVar = 0u;

/* Internal functions definitoin */
static void Comp_Black2_trimAdjust(uint8 nibble) ;

/* static Comp_Black2_backupStruct  Comp_Black2_backup; */
#if (CY_PSOC5A)
    static Comp_Black2_LOWPOWER_BACKUP_STRUCT  Comp_Black2_lowPowerBackup;
#endif /* CY_PSOC5A */

/* variable to decide whether or not to restore the control register in 
   Enable() API for PSoC5A only */
#if (CY_PSOC5A)
    static uint8 Comp_Black2_restoreReg = 0u;
#endif /* CY_PSOC5A */


/*******************************************************************************
* Function Name: Comp_Black2_Init
********************************************************************************
*
* Summary:
...
void Comp_Black2_Init(void) 
{
    ...
    Comp_Black2_SetSpeed(Comp_Black2_DEFAULT_SPEED);

    /* Set default Hysteresis */
    #if ( Comp_Black2_DEFAULT_HYSTERESIS == 0u )
        Comp_Black2_CR |= Comp_Black2_HYST_OFF;
    #else
        Comp_Black2_CR &= (uint8)(~Comp_Black2_HYST_OFF);
    #endif /* Comp_Black2_DEFAULT_HYSTERESIS == 0u */
    /* Power down override feature is not supported for PSoC5A. */
    #if (CY_PSOC3 || CY_PSOC5LP)
        /* Set default Power Down Override */
        #if ( Comp_Black2_DEFAULT_PWRDWN_OVRD == 0u )
            Comp_Black2_CR &= (uint8)(~Comp_Black2_PWRDWN_OVRD);
        #else 
            Comp_Black2_CR |= Comp_Black2_PWRDWN_OVRD;
        #endif /* Comp_Black2_DEFAULT_PWRDWN_OVRD == 0u */
    #endif /* CY_PSOC3 || CY_PSOC5LP */
    
    /* Set mux always on logic */
    Comp_Black2_CR |= Comp_Black2_MX_AO;

    /* Set default sync */
    Comp_Black2_CLK &= (uint8)(~Comp_Black2_SYNCCLK_MASK);
    #if ( Comp_Black2_DEFAULT_BYPASS_SYNC == 0u )
        Comp_Black2_CLK |= Comp_Black2_SYNC_CLK_EN;
    #else
        Comp_Black2_CLK |= Comp_Black2_BYPASS_SYNC;
    #endif /* Comp_Black2_DEFAULT_BYPASS_SYNC == 0u */
}


/*******************************************************************************
* Function Name: Comp_Black2_Enable
********************************************************************************
*
* Summary:
...
{
    Comp_Black2_PWRMGR |= Comp_Black2_ACT_PWR_EN;
    Comp_Black2_STBY_PWRMGR |= Comp_Black2_STBY_PWR_EN;
     
     /* This is to restore the value of register CR which is saved 
    in prior to the modification in stop() API */
    #if (CY_PSOC5A)
        if(Comp_Black2_restoreReg == 1u)
        {
            Comp_Black2_CR = Comp_Black2_lowPowerBackup.compCRReg;

            /* Clear the flag */
            Comp_Black2_restoreReg = 0u;
        }
    #endif /* CY_PSOC5A */
}
...
/*******************************************************************************
* Function Name: Comp_Black2_Start
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  Comp_Black2_initVar: Is modified when this function is called for the 
...
void Comp_Black2_Start(void) 
{
    ...
    if ( Comp_Black2_initVar == 0u )
    {
        Comp_Black2_Init();
        
        Comp_Black2_initVar = 1u;
    }   
...
/*******************************************************************************
* Function Name: Comp_Black2_Stop
********************************************************************************
*
* Summary:
...
void Comp_Black2_Stop(void) 
{
    ...
    Comp_Black2_PWRMGR &= (uint8)(~Comp_Black2_ACT_PWR_EN);
    Comp_Black2_STBY_PWRMGR &= (uint8)(~Comp_Black2_STBY_PWR_EN);
    
    #if (CY_PSOC5A)
        /* Enable the variable */
        Comp_Black2_restoreReg = 1u;

        /* Save the control register before clearing it */
        Comp_Black2_lowPowerBackup.compCRReg = Comp_Black2_CR;
        Comp_Black2_CR = Comp_Black2_COMP_REG_CLR;
    #endif /* CY_PSOC5A */
}


/*******************************************************************************
* Function Name: Comp_Black2_SetSpeed
********************************************************************************
*
* Summary:
...
void Comp_Black2_SetSpeed(uint8 speed) 
{
    ...
    Comp_Black2_CR = (Comp_Black2_CR & (uint8)(~Comp_Black2_PWR_MODE_MASK)) |
                           (speed & Comp_Black2_PWR_MODE_MASK);

    /* Set trim value for high speed comparator */
    if(speed == Comp_Black2_HIGHSPEED)
    {
        /* PSoC5A */
        #if (CY_PSOC5A)
            Comp_Black2_TR = Comp_Black2_HS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            Comp_Black2_TR0 = Comp_Black2_HS_TRIM_TR0;
            Comp_Black2_TR1 = Comp_Black2_HS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
    else
    {
    /* PSoC5A */
        #if (CY_PSOC5A)
            Comp_Black2_TR = Comp_Black2_LS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            Comp_Black2_TR0 = Comp_Black2_LS_TRIM_TR0;
            Comp_Black2_TR1 = Comp_Black2_LS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
...
/*******************************************************************************
* Function Name: Comp_Black2_GetCompare
********************************************************************************
*
* Summary:
...
{
    return( Comp_Black2_WRK & Comp_Black2_CMP_OUT_MASK);
}


/*******************************************************************************
* Function Name: Comp_Black2_trimAdjust
********************************************************************************
*
* Summary:
...
static void Comp_Black2_trimAdjust(uint8 nibble) 
{
    ...
    cmpState = Comp_Black2_WRK & Comp_Black2_CMP_OUT_MASK;
    ...
    {    
        ...
        if ( cmpState != 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                Comp_Black2_TR |= Comp_Black2_CMP_TRIM1_DIR;
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                Comp_Black2_TR0 |= Comp_Black2_CMP_TR0_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    else
    {
        ...
        if ( cmpState == 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                Comp_Black2_TR |= Comp_Black2_CMP_TRIM2_DIR; 
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                Comp_Black2_TR1 |= Comp_Black2_CMP_TR1_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    ...
    for ( trimCnt = 0u; trimCnt < trimCntMax; trimCnt++ )
	{
        ...
        if ((Comp_Black2_WRK & Comp_Black2_CMP_OUT_MASK) != cmpState)
        {
...
/*******************************************************************************
* Function Name: Comp_Black2_ZeroCal
********************************************************************************
*
* Summary:
...
uint16 Comp_Black2_ZeroCal(void) 
{
    ...
    tmpSW0 = Comp_Black2_SW0;
    tmpSW2 = Comp_Black2_SW2;
    tmpSW3 = Comp_Black2_SW3;

     /* Clear routing for inP, retain routing for inN */
    Comp_Black2_SW0 = 0x00u;
    Comp_Black2_SW2 = 0x00u;
    Comp_Black2_SW3 = tmpSW3 & (uint8)(~Comp_Black2_CMP_SW3_INPCTL_MASK);

    /* Preserve original configuration
     * - turn off Hysteresis
     * - set calibration bit - shorts inN to inP
    */
    tmpCR = Comp_Black2_CR;
    Comp_Black2_CR |= (Comp_Black2_CAL_ON | Comp_Black2_HYST_OFF);
    
    /* Write default low values to trim register - no offset adjust */
    /* PSoC5A */
    #if (CY_PSOC5A)
        Comp_Black2_TR = Comp_Black2_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        Comp_Black2_TR0 = Comp_Black2_DEFAULT_CMP_TRIM;
        Comp_Black2_TR1 = Comp_Black2_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC3 || CY_PSOC5LP */
	
	/* Two phase trim - slow modes, one phase trim - for fast */ 
    if ( (Comp_Black2_CR & Comp_Black2_PWR_MODE_MASK) == Comp_Black2_PWR_MODE_FAST)
    {
        Comp_Black2_trimAdjust(0u);
    }
    else /* default to trim for fast modes */
    {
        Comp_Black2_trimAdjust(1u);
		Comp_Black2_trimAdjust(0u);
    }
   
    /* Restore Config Register */
    Comp_Black2_CR = tmpCR;
    
    /* Restore routing registers for inP */
    Comp_Black2_SW0 = tmpSW0;
    Comp_Black2_SW2 = tmpSW2;
    Comp_Black2_SW3 = tmpSW3;
    
    /* PSoC5A */
    #if (CY_PSOC5A)
        return (uint16) Comp_Black2_TR;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        return ((uint16)((uint16)Comp_Black2_TR1 << 8u) | (Comp_Black2_TR0));        
    #endif /* CY_PSOC3 || CY_PSOC5LP */
}


/*******************************************************************************
* Function Name: Comp_Black2_LoadTrim
********************************************************************************
*
* Summary:
...
void Comp_Black2_LoadTrim(uint16 trimVal) 
{
    ...
    /*******************************************************************************
    * Function Name: Comp_Black2_PwrDwnOverrideEnable
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    /*******************************************************************************
    * Function Name: Comp_Black2_PwrDwnOverrideDisable
    ********************************************************************************
    *
    * Summary:
